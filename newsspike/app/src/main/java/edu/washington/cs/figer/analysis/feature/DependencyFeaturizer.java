...
public class DependencyFeaturizer implements AbstractFeaturizer {
	...
	public static HashSet<String> ignorePosTags = new HashSet<String>(), auxVerbs = new HashSet<String>();
		...
		if (X.useReverbFeaturizer) {
			...
			try {
				reader = new BufferedReader(new InputStreamReader(
                                // For Play: put this in a jar, put the jar in /lib
			        new GZIPInputStream(DependencyFeaturizer.class.getResourceAsStream("reverbFeatures.list.gz"))));
				String line = null;
				while ((line = reader.readLine()) != null) {
					reverbPatterns.add(line.split("\t" + "")[1].replaceAll("\\s+", " ").toLowerCase());
				}
				reader.close();
			} catch (IOException e) {
				e.printStackTrace();
	...
	public void apply(Mention m, ArrayList<String> features, Model model) {
		...
		for (Dependency dep : m.getDepsList()) {
			...
			if ((m.getStart() <= dep.getGov() && dep.getGov() < m.getEnd())
					^ (m.getStart() <= dep.getDep() && dep.getDep() < m.getEnd())) {
				if (m.getStart() <= dep.getGov() && dep.getGov() < m.getEnd()) {
					if (dep.getDep() >= m.getPosTagsCount()) {
						X.getCounter("NUM_errors_training_mentions").Increment();
						continue;
					}
					String posTag = m.getPosTags(dep.getDep());// sent.get(dep).split(X.delimS)[X.idxPosS];
					if (acceptDepType(type) && acceptPosTag(posTag)) {
						String t = TokenFeaturizer.getToken(m.getTokens(dep.getDep()), posTag);
						String key = "gov:" + type + ":" + t + "=" + posTag.charAt(0);
				...
				if (m.getStart() <= dep.getDep() && dep.getDep() < m.getEnd()) {
					...
					if (acceptDepType(type) && acceptPosTag(posTag)) {
						String t = TokenFeaturizer.getToken(m.getTokens(dep.getGov()), posTag);
						String key = "dep:" + type + ":" + t + "=" + posTag.charAt(0);
						features.add(("DEP_" + key));

						if (X.useReverbFeaturizer) {
							if (type.equals("nsubj") && posTag.startsWith("V")) {
								ArrayList<String> rvtokens = new ArrayList<String>(), rvpos = new ArrayList<String>();
								for (int k = 1; k < 9 && dep.getGov() + k < m.getTokensCount(); k++) {
									if (!ignorePosTags.contains(m.getPosTags(dep.getGov() + k))) {
										String tk = TokenFeaturizer.getToken(m.getTokens(dep.getGov() + k),
								...
								}
								{
									...
									while (i < lastVerbIndex) {
										...
										if (i + 1 < rvpos.size() && !rvpos.get(i + 1).startsWith("V"))
											break;
								...
								}
								{
									String longest = null;
									if (rvtokens.size() > 0) {
										StringBuilder pat = new StringBuilder(rvtokens.get(0));

										for (int k = 1; k < rvtokens.size(); k++) {
											pat.append(" " + rvtokens.get(k));
											if (reverbPatterns.contains(pat.toString().trim())) {
												longest = pat.toString().trim().replace(" ", "_");
											}
										}
										if (longest != null) {
