...
//	public static String configFile = "data/config/figer.conf";
	public static String configFile = "data/figer.conf";
//	public static String configFile = "figer.conf";
	...
	private ParseStanfordFigerReverb() {
		...
		MapType.typeFile = X.tagset;
                System.out.println("PSFR tagset: " + X.tagset);
		MapType.init();
	...
	public static void callreverb(ReVerbExtractorWrap rew, BinaryExtractionNormalizer normalizer,
		...
		try {
                        ...
                        System.out.println("PSFR tkn length: " + ps.tkn.length);
                        System.out.println("PSFR pos length: " + ps.pos.length);

                        for(String tkn : ps.tkn){
                            System.out.println("PSFR tkn: " + tkn);
                        }

                        for(String pos : ps.pos){
                            System.out.println("PSFR pos: " + pos);
                        }
                        
			ReVerbResult rvr = rew.parse(ps.tkn, ps.pos);
			ChunkedSentence sent = rvr.chunk_sent;

                        System.out.println("PSFR: reverb_extract size: " + rvr.reverb_extract.size());  
...
public static List<FigerParsedSentence> process(String text) {
                ...
                System.out.println("PSFR process: " + text);
			...
			int lineId = 0;
                        ...
                        System.out.println("PSFR annotation sentences: " + sentences.size());
                    	...
                    	for (CoreMap sentence : sentences) {
                       		...
                       		for (Pair<Integer, Integer> offset : entityAllMentionOffsets) {
                                	OneNer ner = new OneNer();
                                        ...
                                        System.out.println("PSFR process: adding block NER");
                                        ps.blockNers.add(ner);
                                }
                                System.out.println("PSFR process: calling reverb");
                                callreverb(rew, normalizer, headnoun_extractor, ps);
                                System.out.println("PSFR process: fps adding ps");
                                fps.add(ps);
				sentId++;
                        }
